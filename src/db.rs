...
pub async fn init() -> Result<Pool<Postgres>, Error> {
    let db_user = dotenv::var(DB_USER_KEY)
        .unwrap_or_else(|_| {
            std::env::var(DB_USER_KEY).expect(format!("{DB_USER_KEY} must be set in env").as_str())
        });
    
    let db_password = dotenv::var(DB_PASSWORD_KEY)
        .unwrap_or_else(|_| {
            std::env::var(DB_PASSWORD_KEY).expect(format!("{DB_PASSWORD_KEY} must be set in env").as_str())
        });
